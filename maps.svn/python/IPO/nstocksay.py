...
nvalue = (((ntotal*10) / ((itotal * 2) + ntotal ) + (ntotal / 100000)) * 222)

#EASILY SETTABLE PARAMETERS
service_charge = 0    # service charges for transactions as a percent
exchange_rate = nvalue # exchange rate of imperial to silver (value 1)
bankdatabase = "navarianroyalStock_DB"
...
thanks_message = [ \
	'Thank you for investing in the Bank of Navar.', \
	'Thank you for investing in the Bank of Navar.', \
...
if text[0] == 'help' or text[0] == 'yes':
	message ='You can:\n-deposit,-withdraw,-balance,-exchange \
		\nAll transactions are in Navarian Stock Certificates\n(1 : %d gold coins). \
	...
	if len(text) == 2:
		amount = int(text[1])
		if amount <= 0:
			message = 'Usage "deposit <amount in Navarian Stock Certificates>"'
		elif amount > 10000:
			message = 'Sorry, we do not accept more than 10000 Navarian Stock Certificates for one deposit.'
		elif activator.PayAmount(int(amount*exchange_rate*fees)):
			bank.deposit(activatorname, amount)
			message = '%d platinum received, %d Navarian Stock Certificates deposited to broker account. %s' \
	...
	if len(text) == 2:
		amount = int(text[1])
		if amount <= 0:
			message = 'Usage "withdraw <amount in Navarian Stock Certificates>"'
		elif amount > 10000:
			message = 'Sorry, we do not accept more than 10000 Navarian Stock Certificates for one withdraw.'
		elif bank.withdraw(activatorname, amount):
			message = '%d Navarian Stock Certificates withdrawn from broker account. %s' \
			%(amount, random.choice(thanks_message))
			id = activator.Map.CreateObject('nstock', x, y)
			CFItemBroker.Item(id).add(amount)
			activator.Take(id)
		else:
			message = 'Not enough Navarian Stock Certificates on your account'
	else:
		message = 'Usage "withdraw <amount in Navarian Stock Certificates>"'
	...
	if len(text) == 2:
		amount = int(text[1])
		if amount <= 0:
			message = 'Usage "exchange <amount>" (Navarian Stock Certificates to platinum coins)'
		elif amount > 10000:
			message = 'Sorry, we do not exchange more than 10000 Navarian Stock Certificates all at once.'
		else:
			inv = activator.CheckInventory('nstock')
			if inv:
				...
				if pay:
					...
					message = 'Sorry, you do not have %d Navarian Stock Certificates'%(amount)
			else:
				message = 'Sorry, you do not have any Navarian Stock Certificates'
	else:
		message = 'Usage "exchange <amount>" (Navarian Stock Certificates to platinum coins)'
	...
	if balance == 1:
		message = 'Amount in broker account: 1 Navarian Stock Certificate'
	elif balance:
		message = 'Amount in broker account: %d Navarian Stock Certificates'%(balance)
	else:
