...
def do_cabin():
	...
	if l.ReadKey('ship_serial') is not '':# Teleports the player in if the map's already made
		...
		template = (str(datadir) + '/' + str(mapdir) + '/template/addition/basic_addition_15x15')# Template for the cabin map.  We could have this randomly picked from a list of template maps, but right now we just have the one template.
	...
	cabexdoor = dest.Check('tree5', (7, 13))# If you want to use a different placeholder item for the cabin exit, change all references to "tree5" to your new arch
...
def do_backup_loc():
	...
	if l.CheckInventory('cabin door').ReadKey('ship_serial') is not '':
		...
		cabexdoor = dest.Check('tree5', (7, 13))
