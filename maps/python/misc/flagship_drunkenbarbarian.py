...
def do_cabin():
	...
	if l.ReadKey('ship_serial') is not '':# Teleports the player in if the map's already made
		...
		template = (str(datadir) + '/' + str(mapdir) + '/ship-cabins/flagship_drunkenbarbarian')# Template for the cabin map.  We could have this randomly picked from a list of template maps, but right now we just have the one template.
